 ...
 * GET  /api/cobranzaClaudeSuggest  -> health check (ver en navegador si la funcion existe)
 *
 * Variables: ANTHROPIC_API_KEY, opcional ANTHROPIC_MODEL, PUBLIC_SUPABASE_URL + PUBLIC_SUPABASE_ANON_KEY
 *
 * Modelo por defecto: alias Haiku 4.5 (rapido / economico). Los viejos p.ej. claude-3-5-haiku-20241022
 * pueden devolver 404 desde Anthropic. Ver https://docs.anthropic.com/en/docs/about-claude/models
 */

const ANTHROPIC_URL = "https://api.anthropic.com/v1/messages";
/** Alias estable a ultimo Haiku 4.5; snapshot fijo alternativo: claude-haiku-4-5-20251001 */
const DEFAULT_MODEL = "claude-haiku-4-5";
const LOG = "[cobranzaClaudeSuggest]";

/** Origenes permitidos para Credito / Cobranza+IA (mismo build en marketing o en hecom.club via proxy u origen distinto). */
...
async function verifySupabaseSession(req) {
  ...
  if (!auth || !/^Bearer\s+\S+/i.test(auth)) {
    return { ok: false, status: 401, error: "Falta sesion. Volve a entrar al panel e intenta de nuevo." };
  }
  try {
    ...
    await r.json();
    console.log(LOG, "auth: sesion OK");
    return { ok: true };
  } catch (e) {
    console.error(LOG, "auth: fetch error", e?.message || e);
    return { ok: false, status: 503, error: "No se pudo validar la sesion." };
  }
}
...
function buildUserPrompt(body, agenciaNombre) {
  ...
  const reglaMarca = `OBLIGATORIO - Agencia que envia el correo: "${marcaAgencia}".
- El cuerpo HTML debe incluir al menos una vez la cadena exacta ${marcaAgencia} (sin abreviar ni sustituir por "nosotros" solo).
- Frases utiles: "desde ${marcaAgencia}", "el equipo de ${marcaAgencia}", "en ${marcaAgencia}".
...
- "${empresa || "(sin dato)"}" es el negocio del CLIENTE; no lo uses como nombre de la agencia.`;
  ...
  if (tipo === "agradecimiento") {
    return `Genera un correo en espanol (Peru/Latam, voseo o tu segun suene natural).

Tipo: AGRADECIMIENTO - el cliente esta al dia, sin saldo pendiente segun nuestros registros.
Cliente: ${nombre}${empresa ? `\nEmpresa o marca del cliente (su negocio): ${empresa}` : ""}
Contexto del periodo: ${periodo || "Cuenta al dia."}

${reglaMarca}

Requisitos:
- Tono profesional, breve y humano (no frio ni amenazante).
...
- No incluyas firma de empresa al final (la agrega el sistema automaticamente).

Responde UNICAMENTE con un JSON valido en una sola linea o bloque, sin texto antes ni despues, con esta forma exacta:
{"subject":"asunto corto para la bandeja","bodyHtml":"<p>...</p>"}`;
  }

  return `Genera un correo en espanol (Peru/Latam).

Tipo: COBRO / recordatorio de saldo pendiente.
Cliente: ${nombre}${empresa ? `\nEmpresa o marca del cliente (su negocio): ${empresa}` : ""}
Monto pendiente: ${moneda} ${monto}
Contexto del periodo o deuda: ${periodo || "Saldo pendiente segun cuenta."}

${reglaMarca}

Requisitos:
- Tono firme pero respetuoso; recorda el pago sin sonar agresivo.
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    return res.status(200).json({
      ok: true,
      message: "Endpoint activo. Usa POST con JSON desde Credito -> Cobranza -> Personalizar con IA.",
  ...
  if (!apiKey) {
    ...
    return res.status(503).json({
      success: false,
      error:
        "ANTHROPIC_API_KEY no esta configurada en Vercel. Agregala en Project -> Settings -> Environment Variables (Production).",
    });
  }
  ...
  try {
    ...
    const response = await fetch(ANTHROPIC_URL, {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1200,
        system:
          'Sos un asistente para redactar correos de una agencia de marketing. Si el prompt da un nombre de agencia entre comillas, el bodyHtml debe contener ese nombre textual al menos una vez; no lo omitas por brevedad. Responde solo con JSON: {"subject":"...","bodyHtml":"..."}. bodyHtml: solo <p>, <strong> y <br>. Sin markdown fuera del JSON.',
    ...
    if (!text) {
      ...
      return res.status(502).json({ success: false, error: "Respuesta vacia de Claude." });
    }
    ...
    try {
      ...
      return res.status(502).json({
        success: false,
        error: "Claude no devolvio JSON valido. Proba de nuevo o acorta el contexto.",
      });
    }
    ...
    console.log(LOG, "exito", { subjectLen: subject.length, bodyLen: bodyHtml.length });
    return res.status(200).json({ success: true, data: { subject, bodyHtml } });
  } catch (err) {
    console.error(LOG, "excepcion", err?.stack || err?.message || err);
